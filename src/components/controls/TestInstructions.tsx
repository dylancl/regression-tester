  ...
  return (
    <Box sx={{ overflowX: 'auto' }}>
      <Box sx={{ 
        ...
        <Box sx={{ display: 'table-header-group', bgcolor: theme.palette.action.hover }}>
...
const TestInstructions: React.FC<TestInstructionsProps> = ({ 
  ...
  const fetchScenarios = useCallback(async () => {
    // No need to check if sidebar is open, as component will always be visible
    setLoading(true);
    setError(null);
    try {
  ...
  const getScenarioProgress = useCallback((scenarioId: string, steps: TestStep[]): { 
    completion: number; 
    passed: number; 
    failed: number;
    blocked: number;
  ...
  const setStepStatus = useCallback((scenarioId: string, stepId: string, status: TestStepStatus) => {
    ...
    // Show notification
    setNotification(`Step ${status === 'pass' ? 'passed' : status === 'fail' ? 'failed' : 'blocked'}`);
  ...
  // Get status color
  const getStatusColor = useCallback((status: TestStepStatus) => {
    switch (status) {
  ...
  const renderFab = useCallback(() => (
    <Tooltip title={sidebarOpen ? "Hide Test Instructions" : "Show Test Instructions"}>
      <Fab
        color="primary"
  ...
  ), [sidebarOpen, toggleSidebar, progressData]);
  ...
  // Render test step
  const renderTestStep = useCallback((scenario: TestScenario, step: TestStep, index: number) => {
    const stepKey = `${scenario.id}-${step.id}`;
    const status = stepStatuses[stepKey];
    ...
    return (
      <ListItem
        ...
        sx={{
          ...
          py: 1.5,
          px: 2,
          borderBottom: index < scenario.steps.length - 1 ? `1px solid ${theme.palette.divider}` : 'none',
          '&:hover': {
            ...
            <Box sx={{ 
              ...
              <Typography
                ...
                sx={{
                  ...
                  color: status === 'blocked' ? theme.palette.text.disabled : 'inherit',
              ...
              >
                <Box component="span" sx={{ mr: 1, color: theme.palette.text.secondary, fontSize: '0.8rem', opacity: 0.7 }}>
                  {index + 1}.
                </Box>
                {step.instruction}
              </Typography>
              
              <Tooltip title={showExpected ? "Hide expected result" : "Show expected result"}>
                <IconButton
                  size="small"
                  ...
                  sx={{ 
                    ml: 1, 
                    mt: -0.5,
                    color: showExpected ? theme.palette.primary.main : theme.palette.text.secondary,
                    transition: 'all 0.2s ease',
  ...
  }, [stepStatuses, showExpectedResults, theme, getStatusColor, toggleExpectedResult, setStepStatus]);
    ...
    <>
      <Box sx={{ 
        ...
        <Box>
          ...
          <IconButton onClick={toggleSidebar} edge="end" aria-label="close" size="small">
        ...
        {loading ? (
          ...
          <Box sx={{ 
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 3, maxWidth: 500 }}>
              ...
              You can add test scenarios in the Google Sheet using our two-sheet format.
            </Typography>
  ...
  return (
    <>
      ...
      {/* Button to toggle sidebar when closed - moved outside the Drawer component */}
      {!sidebarOpen && (
