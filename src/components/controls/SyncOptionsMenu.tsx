...
const syncableSettings = [
    { key: 'environment', label: 'Environment', description: 'Deployment environment (dev, acc, prev, prod)' },
    { key: 'component', label: 'Component', description: 'Component type (car-filter, used-stock-cars, etc.)' },
    { key: 'uscContext', label: 'USC Context', description: 'Used or Stock vehicles context' },
    { key: 'uscEnv', label: 'USC Environment', description: 'Backend environment (UAT, Production)' },
    { key: 'brand', label: 'Brand', description: 'Main brand (Toyota, Lexus)' },
    { key: 'variantBrand', label: 'Variant Brand', description: 'Variant brand displayed in components' },
    { key: 'country', label: 'Country', description: 'Country and language selection' },
];
...
export const SyncOptionsMenu: React.FC<SyncOptionsMenuProps> = ({
    ...
    const handleToggleChange = (optionName: string) => (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        >
            ...
            <DialogContent>
                <Typography variant="body2" color="text.secondary" component="p" sx={{ mt: 2 }}>
                    Choose which settings should be synchronized between frames when sync is enabled.
                    Disabled settings will remain independent for each frame.
                </Typography>
